...
class SecretKey(AbstractSecretKey):
    ...
    def decrypt(self, x):
        ...
        elif (type(x) == PaillierTensor):
            return self.decrypt(x.data)
    ...
    def transform(self,x):
        if(type(x) == Float):
            ...
            return self.sk.transform(x)
        elif(type(x) == PaillierTensor):
            #print('hereiam',type(x.data))
    ...
    def finalDecrypt(self, x):
        ...
        if(type(x) == Float):
            ...
            if(x.encrypted):
                ...
                return NotImplemented
            #return PaillierTensor(x.public_key,self.finalDecrypt(x.data),False)
        elif (type(x) == PaillierTensor):
            #print('hereeeeeeeeeeeeeeeyyyy',type(x.data))
            return self.decrypt(x.data)
        elif(type(x) == np.ndarray):
            sh = x.shape
            x_ = x.reshape(-1)
            ...
            for v in x_:
                if (type(v) == EncryptedNumber):
                    out.append(self.sk.finalDecrypt(v))
                    #print('vvvvvv',type(v))
                else:
                    out.append(self.sk.decrypt(v.data))
            return np.array(out).reshape(sh)
