...
class LinearClassifier():
    ...
    def evaluate(self, inputs, targets):
        ...
        targets = TensorBase(np.array(target).astype('float64'))
        inputs = TensorBase(np.array(input).astype('float64'))
        ...
        if(self.encrypted):
            ...
            return int((loss* scale) / float(len(inputs)))
    ...
    def generate_gradient(self, input, target):
        ...
        #print("shape data", input.size())
        pred = TensorBase(self.forward(input))
    ...
    def generate_encrypted_gradient(self, input, target,pubkey):
        target = TensorBase(np.array(target).astype('float64'))
        input = TensorBase(np.array(input).astype('float64'))
        ...
        weight_grad = weight_grad.encrypt(pubkey)
