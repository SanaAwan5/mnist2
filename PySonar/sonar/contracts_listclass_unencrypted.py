...
class Gradient_List():
    ...
    def generate_gradient_avg(self,agg_addr,transform_key,alpha = 0.5):
        length = len(self)
        num_gradients = self.repo.call.getNumGradientsforModel(self.model_id)
        gradient_id = 0
        if num_gradients > 0:
            for i in range (0,num_gradients):
                ...
                self.gradient_list.append(g)
                gradient_id = gradient_id + 1
                print("length of gradient list is", length)
        else:
            print ("gradient list is empty") 
        #self.gradient_list[0].grad_values.data = self.gradient_list[0].grad_values.data;
        print('typeeeee',type(self.gradient_list[0].grad_values))
        for i in range(1,length):
            self.gradient_list[0].grad_values += self.gradient_list[i].grad_values
        #self.gradient_list[0].grad_values = self.gradient_list[0].grad_values /length
        ...
        avg = self.gradient_list[0].grad_values /len(self.gradient_list[0].grad_values)
        #print ("type of avg",type(avg))
        avg=np.array(avg)
        out=list()
        sh=avg.shape
        #print ("shape of avg",np.shape(avg))
        avged_result = []
        #print('avgedresult',type(avg))
        if(type(avg) == np.ndarray):
            avg_ = avg.reshape(-1)
            for v in avg_:
                #print('transformvvvvbefore',type(v))
                
                #print('typeofout',type(v.data))
                temp=transform_key.sk.transform(v.data)
                out.append(temp)
...
class Model():
    def __init__(self, owner, syft_obj, bounty, initial_error, target_error,best_error,
        ...
        self.best_error = initial_error  # TODO: get this
    ...
    def generate_gradient(self, owner,prikey, input, target,alpha,i):
        #num_epochs = 50
        if (self.syft_obj.encrypted):
            self.syft_obj.decrypt(prikey)
        candidate=copy.deepcopy(self.syft_obj)
        #for e in range (num_epochs):
        grad_values,len_n = self.syft_obj.learn(input, target,i)
            #alpha = alpha * 0.09
        #candidate.weights -= grad_values * alpha
        gradient = Gradient(owner, grad_values, None)
        print('gradvaltype',type(grad_values))
        return gradient,candidate,len_n
    ...
    def submit_transformed_gradients(self,gradient,pubkey,st):
        gradient.grad_values = gradient.grad_values.encrypt(pubkey)
        #print('type grad_values',type(gradient.grad_values))
    ...
    def evaluate_gradient(self,addr,gradient,prikey,pubkey, inputs, targets, alpha=1):
        ...
        candidate = copy.deepcopy(self.syft_obj)
        id=self.repo.call.getNumGradientsforModel(self.model_id) - 1
        print("gradientid",id)
        ...
        if (new_model_error <= self.best_error or self.target_error <= self.best_error):
        ...
        #print("tx",tx)
        candidate=candidate.encrypt(pubkey)
        ipfs_address = self.repo.ipfs.store(candidate)
        ip=IPFSAddress().to_ethereum(ipfs_address)
       # self.repo.call.evalGradient(id, new_model_error,ip)
        tx.evalGradient(id, new_model_error,ip)
    ...
    def decrypt_avg(self,transform_key):
        length = self.repo.call.getNumGradientsforModel(self.model_id)
        nwa = self.repo.call.getAvg(self.model_id,length)
        new_avg = self.repo.ipfs.retrieve(IPFSAddress().from_ethereum(nwa))
        #print('avggrad is', type(avg))
    ...
    def update(self,addr,new_model_error,candidate,pubkey):
        if (new_model_error <= self.best_error and self.target_error <= self.best_error):
            totalError = self.initial_error - self.target_error
            solvedError = self.best_error - new_model_error
            self.best_error = new_model_error
...
class ModelRepository():
    ...
    currently assumes we are running on a local testrpc Ethereum blockchain."""

    def __init__(self, contract_address, account=None,
                 ipfs=IPFS('127.0.0.1', 5001),
                 web3_host='localhost', web3_port=7545):
        ...
        self.web3 = Web3(HTTPProvider('http://127.0.0.1:7545'))
